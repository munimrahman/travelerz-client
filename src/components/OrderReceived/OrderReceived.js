...
const OrderReceived = () => {
  ...
  // console.log(orderID);
  useEffect(() => {
    fetch(`https://travelerz-server.vercel.app/orders/${orderID}`)
      .then((res) => res.json())
  ...
  return (
    <div className="bg-eee py-4">
      <div className="container rounded bg-white shadow py-4 px-4">
        ...
        {orderStatus === "CONFIRM" ? (
          <p>
            Welcome! Your Order is &nbsp;
            <span className="badge bg-success">CONFIRMED</span>!
          </p>
        ) : (
          <p>
            Thank you. Your Order is &nbsp;
            <span className="badge bg-warning">PENDING</span>. Please Wait for
            Confirmation!
          </p>
        )}
        <div className="d-flex">
          ...
          <div className="ps-3">
            ...
            {paymentMethod?.pay === "cash" && (
              <p className="mb-0">Cash Payment</p>
            )}
            {paymentMethod?.pay === "bank" && (
        ...
        <table className="table my-4">
          <tbody>
            ...
            <tr>
              ...
              {paymentMethod?.pay === "cash" && (
                <td className="text-center">Cash Payment</td>
              )}
              {paymentMethod?.pay === "bank" && (
                <td className="text-center">Direct Bank Transfer</td>
              )}
              {paymentMethod?.pay === "card" && (
